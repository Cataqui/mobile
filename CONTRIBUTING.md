# Contributing to Cataquí Mobile

Thank you for helping make nearby work easier to discover.

Start with the setup and workspace commands in [README.md](README.md), then
follow the engineering rules in [AGENTS.md](AGENTS.md) and any more-specific
`AGENTS.md` in the directory you change.

## Before you contribute

Cataquí Mobile is **source-available**, not open source as defined by the Open
Source Initiative. The [PolyForm Shield License 1.0.0](LICENSE) lets you inspect,
modify, and share the software for permitted purposes, but it does not permit
using it to provide a competing product. A product may compete even when it is
free.

The license covers Cataquí-authored material only. Do not submit code, assets,
translations, or other material unless you have the right to license it.

## Contributor agreement

Every contributor must accept the
[Contributor License Agreement](CONTRIBUTOR_LICENSE_AGREEMENT.md) when opening
their first pull request. The agreement:

- keeps ownership of your contribution with you;
- gives the project the copyright and patent permissions needed to use,
  protect, and continue licensing the combined work; and
- confirms that you have authority to make the contribution.

Pull requests without a valid acceptance cannot be merged. If your employer or
another organization owns the contribution, an authorized representative of
that organization must accept the agreement. The contributor-agreement check
records the pull request acceptance and legal name; editing the pull request
re-runs the check.

## Submit a change

1. Create a focused branch from `main`.
2. Make the smallest change that fully solves the issue.
3. Run the relevant workspace formatting, analysis, and tests from the
   repository root.
4. Open a pull request and complete every item in the pull request template.

By submitting a pull request, issue attachment, patch, or other material
intended for inclusion, you agree that the submission is a Contribution under
the Contributor License Agreement.

## Third-party material

Identify third-party material in the pull request and preserve its copyright,
license, and notice files. Do not copy code from a project with unclear terms
or from a license incompatible with Cataquí's distribution model.

Maintainers may ask for provenance or reject a contribution when its ownership
or licensing is unclear.
