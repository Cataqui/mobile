...
void main() {
  group('QuiButton', () {
    group('tap behavior', () {
      ...
      testWidgets(
        'when enabled, it should wrap in QuiTap with scale animation',
        (tester) async {
          await tester.pumpWidget(
            TestApp(
              child: QuiButton(
          ...
          final animation = tester.widget<QuiTap>(
            find.descendant(
              of: find.byType(QuiButton),
              matching: find.byType(QuiTap),
            ),
          );
      ...
      testWidgets('when disabled, it should pass null onPressed to QuiTap', (
        tester,
      ) async {
        await tester.pumpWidget(
          const TestApp(
    ...
    group('layout and sizing', () {
      testWidgets('when fit is fit, it should size to content', (tester) async {
        ...
        final buttonWidth = tester
            .getSize(find.byKey(const Key('qui_button_container')))
            .width;
      ...
      testWidgets('when fit is expand, it should fill the available width', (
        ...
        expect(
          tester.getSize(find.byKey(const Key('qui_button_container'))).width,
          equals(300),
        );
      });
      ...
      testWidgets(
        ...
        (tester) async {
          ...
          final gesture = await tester.startGesture(
            tester.getCenter(find.byType(QuiButton)),
          );
          await tester.pump();
          await gesture.up();
          await tester.pump();
...
EdgeInsetsGeometry? _buttonPadding(WidgetTester tester) {
  return tester
      .widget<Padding>(find.byKey(const Key('qui_button_container')))
      .padding;
}
...
QuiDotLoadingIndicator _loadingIndicator(WidgetTester tester) {
  return tester.widget<QuiDotLoadingIndicator>(
    find.byType(QuiDotLoadingIndicator),
  );
}
