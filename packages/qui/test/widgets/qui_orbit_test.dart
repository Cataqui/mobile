...
void main() {
  group('QuiOrbit constructor', () {
    ...
    test('when constructed with 8 items, it should not throw', () {
      expect(
        () => QuiOrbit(
          items: List.generate(8, (_) => const QuiOrbitItem(child: SizedBox(), size: Size(10, 10))),
        ),
        returnsNormally,
      );
    });
  ...
  group('QuiOrbit rendering', () {
    ...
    testWidgets('when rotateItems is true, it should render without error', (tester) async {
      await tester.pumpWidget(TestApp(child: _fourItemOrbit(rotateItems: true)));
    ...
    testWidgets('when rotateItems is true, it should rotate through the paint-only Flow delegate', (tester) async {
      await tester.pumpWidget(TestApp(child: _fourItemOrbit(rotateItems: true)));
    ...
    testWidgets('when animations are disabled and rotateItems is true, no Transform.rotate should exist', (
      tester,
    ) async {
      await tester.pumpWidget(
        MediaQuery(
    ...
    testWidgets('when direction is counterclockwise, it should render without error', (tester) async {
      await tester.pumpWidget(TestApp(child: _fourItemOrbit(direction: QuiOrbitDirection.counterclockwise)));
    ...
    testWidgets('when revolutionDuration is changed, it should rebuild without error', (tester) async {
      ...
      await tester.pumpWidget(TestApp(child: _fourItemOrbit(revolutionDuration: const Duration(seconds: 10))));

      expect(tester.takeException(), isNull);
    });

    testWidgets('when revolutionDuration is changed to a very short duration, it should render without error', (
      tester,
    ) async {
      await tester.pumpWidget(TestApp(child: _fourItemOrbit(revolutionDuration: const Duration(milliseconds: 100))));
    ...
    testWidgets('when placed in a constrained box, it should adapt its size', (tester) async {
      await tester.pumpWidget(
        MediaQuery(
          data: const MediaQueryData(disableAnimations: true),
          child: TestApp(child: SizedBox(width: 200, height: 200, child: _fourItemOrbit())),
        ),
      );
    ...
    testWidgets('when placed in a small box, it should still render', (tester) async {
      await tester.pumpWidget(
        MediaQuery(
          data: const MediaQueryData(disableAnimations: true),
          child: TestApp(child: SizedBox(width: 100, height: 100, child: _fourItemOrbit())),
        ),
      );
...
Widget _fourItemOrbit({
  ...
  return QuiOrbit(
    ...
    direction: direction,
    items: const [
      QuiOrbitItem(child: _TestIcon(Icons.bolt_rounded, Color(0xFFFF4A4B)), size: Size(48, 48)),
      QuiOrbitItem(child: _TestIcon(Icons.restaurant_rounded, Color(0xFF00A896)), size: Size(48, 48)),
      QuiOrbitItem(child: _TestIcon(Icons.delivery_dining_rounded, Color(0xFF3D5A80)), size: Size(48, 48)),
      QuiOrbitItem(child: _TestIcon(Icons.cleaning_services_rounded, Color(0xFFF4A261)), size: Size(48, 48)),
    ],
  );
}
