...
class _PostLocationViewSearchBody extends ConsumerWidget {
  const _PostLocationViewSearchBody({required this.onAddressSelected});

  final ValueChanged<AddressSuggestionDto> onAddressSelected;

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final i18n = ref.watch(translationProvider);

    ref.listen<bool>(postLocationStateProvider.select((locationData) => locationData.addressSearch.isLoading), (
      wasLoading,
      isLoading,
    ) {
  ...
  Widget _buildErrorSearchMessage(BuildContext context, {required String message}) {
    ...
    return Center(
      key: const ValueKey('post_location_search_error_transition'),
      child: Padding(
          ...
          children: [
            ExcludeSemantics(
              child: $Lotties.doubleCross(
                key: const ValueKey('post_location_search_error_animation'),
                width: 56,
                height: 56,
  ...
  Widget _buildEmptySearchMessage(BuildContext context, {required String message}) {
    return Center(
      key: const ValueKey('post_location_search_empty_transition'),
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 20).copyWith(top: 42),
        child: Column(
          mainAxisSize: MainAxisSize.min,
  ...
  Widget _buildOfflineSearchMessage(BuildContext context, {required String message}) {
    ...
    return Center(
      key: const ValueKey('post_location_search_offline_error_transition'),
      child: Padding(
        padding: const EdgeInsets.symmetric(horizontal: 20).copyWith(top: 42),
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            ExcludeSemantics(
              child: $Lotties.wifiSignalPop(
                key: const ValueKey('post_location_search_offline_animation'),
                width: 56,
                height: 56,
