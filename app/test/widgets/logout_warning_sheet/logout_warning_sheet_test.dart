...
void main() {
  ...
  testWidgets('the warning icon shakes only after the sheet route settles', (tester) async {
    ...
    expect(
      ...
      (MotionStartup.skip, const Offset(2, 0), 4, 1, const Duration(milliseconds: 1700), Curves.easeOutBack),
    );
  ...
  testWidgets('the sheet close button dismisses the warning', (tester) async {
    var closed = false;
    await tester.pumpWidget(TestApp.screen(child: LogoutWarningSheetTestHost(onClosed: () => closed = true)));
    await tester.tap(find.byKey(LogoutWarningSheetTestHost.openButtonKey));
    await tester.pumpAndSettle();
  ...
  testWidgets('Vou sair leaves the warning open and does not log out', (tester) async {
    var closed = false;
    await tester.pumpWidget(TestApp.screen(child: LogoutWarningSheetTestHost(onClosed: () => closed = true)));
    await tester.tap(find.byKey(LogoutWarningSheetTestHost.openButtonKey));
    await tester.pumpAndSettle();
