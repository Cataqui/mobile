...
void main() {
  ...
  group('FeedView map sequencing', () {
    testWidgets(
      'when the feed loads with multiple jobs, it should mount only the first card MapLibreMap',
      (tester) async {
        await FeedViewTestHelpers.pumpFeedView(
          tester: tester,
          feedState: FakeFeedState(
            buildResult: () => FeedViewTestHelpers.feedDataWithJobs(count: 3),
          ),
        );
    ...
    testWidgets(
      'when the first map fires onMapIdle, it should mount the second card MapLibreMap',
      (tester) async {
        await FeedViewTestHelpers.pumpFeedView(
          tester: tester,
          feedState: FakeFeedState(
    ...
    testWidgets(
      'when swiping before onMapIdle fires, the onNext fallback should mount the next card map',
      (tester) async {
        await FeedViewTestHelpers.pumpFeedView(
          tester: tester,
          feedState: FakeFeedState(
    ...
    testWidgets(
      'when swiping to a card that already has a mounted map, _maxMapIndex should not regress',
      (tester) async {
        await FeedViewTestHelpers.pumpFeedView(
          tester: tester,
          feedState: FakeFeedState(
